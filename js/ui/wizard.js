...
// ──── Step 2: Inverter ────
function rendS2(inv) {
  const svRange = inv.voltageOverridden ? (inv.logicalVoltage + 'V / ' + inv.systemVoltage + 'V') : getVoltageRangeSimple(inv.peakPower);
  const svNote = inv.voltageOverridden ? '<p class="text-xs text-orange-600 mt-1">' + inv.logicalVoltage + 'V by Peak Power rule, adjusted to ' + inv.systemVoltage + 'V to match available inverter</p>' : '';
  ...
  h += rC('battery', 'System Voltage', svRange + svNote, '', 'blue');
  h += '</div>';
  ...
  h += '<div class="info-box"><p>\u2022 Peak Power: Surge loads \u00d72 + Non-surge \u00d71.2 (or Total \u00d71.2)</p><p>\u2022 ' + getVoltageGuideText(state.voltageStandard) + '</p>' + (inv.voltageOverridden ? '<p>\u2022 Voltage adjusted from ' + inv.logicalVoltage + 'V to ' + inv.systemVoltage + 'V \u2014 no ' + inv.logicalVoltage + 'V inverter matches your power range. Battery recommendations follow ' + inv.systemVoltage + 'V.</p>' : '') + '</div></div>';
  return h;
}
...
function rendS4(inv, bat) {
  let h = '<div class="space-y-6"><div><h2 class="text-xl font-bold">Your Complete System</h2></div><div class="summary-card"><div class="accent-shape"></div><div class="summary-title"><svg class="icon-lg text-orange-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="9" y1="21" x2="9" y2="9"/></svg>Configuration Summary</div><div class="summary-grid mt-4"><div class="summary-item orange"><p class="summary-item-label">System Voltage</p><p class="summary-item-value text-orange-700">' + inv.systemVoltage + 'V</p>' + (inv.voltageOverridden ? '<p class="summary-item-sublabel">Adjusted from ' + inv.logicalVoltage + 'V (no matching inverter)</p>' : '') + '</div><div class="summary-item orange"><p class="summary-item-label">Inverter System</p><p class="summary-item-value text-orange-700">' + (inv.inverterSize >= 1000 ? (inv.inverterSize / 1000).toFixed(1) + 'kW' : inv.inverterSize + 'W') + (inv.isParallel ? ' <span style="font-size:14px;font-weight:500">(Parallel)</span>' : '') + '</p></div><div class="summary-item green"><p class="summary-item-label">Battery Storage System</p><p class="summary-item-value text-green-700">' + bat.batteryKwh.toFixed(1) + 'kWh</p><p class="summary-item-sublabel">' + Math.round(bat.requiredAh) + 'Ah</p></div></div></div>';
