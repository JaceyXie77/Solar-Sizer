...
function rendS1(trp) {
  ...
  h += '<div class="flex flex-wrap items-center gap-4"><label class="whitespace-nowrap">What do you want to power?</label><select onchange="appP(this.value)"><option value="" disabled selected>Select one</option>' + PRESET_OPTIONS.map(function(o) { return '<option value="' + o.value + '" ' + (state.currentPreset === o.value ? 'selected' : '') + '>' + o.label + '</option>'; }).join('') + '</select></div>';
  h += '<div class="flex flex-wrap items-center gap-4"><label class="whitespace-nowrap">Voltage Standard</label><select onchange="setVS(this.value)">' + Object.entries(VOLTAGE_LABELS).map(function(kv) { return '<option value="' + kv[0] + '" ' + (state.voltageStandard === kv[0] ? 'selected' : '') + '>' + kv[1] + '</option>'; }).join('') + '</select></div>';
  h += rendLT('Single-phase Appliances', it, true);
  h += '<div class="flex items-center justify-between"><button class="btn btn-sm" onclick="addLI()"><svg class="icon-md mr-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Add Appliance</button><div class="total-badge">' + tBG(sr, 'Single-phase Total') + '</div></div>';
